...
struct LocationPickerView: View {
    ...
    private var location: CLLocationCoordinate2D? { userLocation.location }
            ...
            let longitude = location?.longitude
        {
            ...
            Button(
                action: {
                    ...
                    routerCoordinator.navigate(
                        to: .parent(route: .weatherStatus(location: location)))
                },
                label: {
...
#Preview {
    LocationPickerView(
        routerCoordinator: LocationRouterCoordinator(
            parent: MainRouterCoordinator()))
}
